...
- The board renders as markdown rows (`"renderStyle": "rows"`): bold header line, then per player `🟡 #1 @mention · <rank emoji> Rank · Peak · Role · live timestamp`. Position badges are gold/blue/green for the top 3, white for the rest. Rank emoji entries in `tierEmoji` that look like `":grandmaster:"` are resolved to the server's custom emojis at runtime (fallback 🔹 if missing); plain unicode emoji are used as-is. Sorting: current rank → peak rank → earliest last-update. Setting `"renderStyle": "ansi"` switches to a colored monospace table instead (aligned columns, but no custom emoji, mentions or live timestamps — Discord limitation).
