...
- The board renders as a 3-column grid (`"renderStyle": "table"`, the default): bold column headers `# · Player | Rank · Peak | Role · Updated`, one aligned row per player with gold/blue/green position badges for the top 3 (white for the rest), custom rank emoji, a real @mention, and a live relative timestamp — plus an empty spacer row before the footer. On mobile, Discord stacks the three columns vertically (platform behaviour, not configurable). Rank emoji entries in `tierEmoji` that look like `":grandmaster:"` are resolved to the server's custom emojis at runtime (fallback 🔹 if missing); plain unicode emoji are used as-is. Sorting: current rank → peak rank → earliest last-update. Other styles: `"rows"` = one markdown line per player; `"ansi"` = colored monospace code-block table (aligned columns, but no custom emoji, mentions or live timestamps — Discord limitation).
